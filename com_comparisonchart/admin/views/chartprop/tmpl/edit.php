...
$filter_chart=$app->getUserState( 'filter_chart');
...
<script type="text/javascript">
    ...
    {
        if (task == 'chartprop.cancel' || document.formvalidator.isValid(document.id('item-form'))) {
    ...
    $(document).ready(function() {
        <?php if(intval($filter_chart)){?>
        var chart_id=<?php echo $filter_chart; ?>;
        $('#filter_chart').val(chart_id);
        <?php }else{?>
        var chart_id=$('#jform_chart_id option:selected').val();
        $('#filter_chart').val(chart_id);
        <?php }?>
...
<form action="<?php echo JRoute::_('index.php?option=com_comparisonchart&layout=edit&id=' . (int) $this->item->id); ?>" method="post" name="adminForm" id="item-form" class="form-validate">
    <div class="row-fluid">
        ...
        <div class="span10 form-horizontal">
            ...
            <input type="hidden" name="filter_chart" value="" id="filter_chart">
